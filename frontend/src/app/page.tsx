...
import { useAuth, useSignIn, SignInButton, SignUpButton } from '@clerk/nextjs';
...
function PublicLandingPage() {
  ...
  const handleOAuthLogin = async (strategy: 'oauth_google' | 'oauth_github') => {
    ...
    try {
      ...
      console.warn('OAuth redirect failed, falling back:', err);
        ...
        <div className="max-w-2xl text-center space-y-3 md:space-y-4 my-auto">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2.5 pt-2">
            <button
              ...
              className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-slate-900/90 border border-slate-700 hover:border-slate-500 hover:bg-slate-800 text-slate-200 text-xs font-semibold shadow-md transition-all cursor-pointer hover:translate-y-[-1px] disabled:opacity-50"
            >
              ...
              <span>{oauthLoading === 'oauth_google' ? 'Redirecting to Google...' : 'Continue with Google'}</span>
            </button>
            ...
            >
              ...
              <span>{oauthLoading === 'oauth_github' ? 'Redirecting to GitHub...' : 'Continue with GitHub'}</span>
            </button>
